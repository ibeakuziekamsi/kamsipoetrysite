(function () {
    ...
    /* poem tools: size, favorite, share */
    if (poem) {
        ...
        function poemSpokenText() {
            return (poem.textContent || "").replace(/\s+/g, " ").replace(/---/g, "").trim();
        }
        function stopListen() {
    ...
    function makePoemCard() {
        ...
        ctx.lineWidth = 2;
        ctx.strokeRect(68, 68, 944, 944);

        ctx.fillStyle = "#FBBF24";
        ctx.font = "28px Georgia, serif";
        ctx.fillText("MIDNIGHT & GOLD", 120, 160);

        ctx.fillStyle = "#F59E0B";
        ctx.font = "italic 56px Georgia, serif";
        ctx.fillText("“", 110, 300);

        ctx.fillStyle = "#FFFBEB";
        ctx.font = "italic 44px Georgia, serif";
        wrapCanvasText(ctx, excerpt, 160, 360, 760, 64);

        ctx.fillStyle = "#FBBF24";
        ctx.font = "32px Georgia, serif";
        ctx.fillText("— Ibeakuzie Kamsiriochukwu", 160, 780);

        ctx.fillStyle = "rgba(226,232,240,0.7)";
        ctx.font = "28px Georgia, serif";
        ctx.fillText(title, 160, 860);

        ctx.fillStyle = "rgba(251,191,36,0.8)";
        ctx.font = "24px Georgia, serif";
        ctx.fillText("ibeakuziekamsi.netlify.app", 160, 960);
    ...
    function openPoemCard() {
        ...
        if (!modal) {
            ...
            modal.innerHTML = '<div class="mg-card-box">' +
        ...
        box.onclick = function (e) {
            ...
            if (act === "copy" && navigator.clipboard) {
                ...
                setTimeout(function () { e.target.textContent = "Copy link"; }, 1600);
            }
            if (act === "share") {
                canvasToFile(canvas, title.replace(/\s+/g, "-").toLowerCase() + "-card.png").then(function (file) {
                    const data = { title: title + " — Ibeakuzie Kamsiriochukwu", text: excerptFromPoem() + "\n— Ibeakuzie Kamsiriochukwu", url: url };
                    if (navigator.canShare && navigator.canShare({ files: [file] })) {
                        return navigator.share({ title: data.title, text: data.text, url: data.url, files: [file] });
                    }
    ...
    if (grid && pageName() === "poems.html") {
        ...
        wrap.innerHTML = '<input class="mg-search" type="search" placeholder="Search poems…">' +
            '<div class="mg-tools"><button type="button" class="mg-btn" id="mg-all">All</button>' +
            '<button type="button" class="mg-btn" id="mg-saved">★ Saved</button></div>' +
    ...
    document.querySelectorAll(".quote-card, .quote-text").forEach(function (el) {
        ...
        el.addEventListener("click", function () {
            const textEl = el.classList.contains("quote-text") ? el : el.querySelector(".quote-text") || el;
